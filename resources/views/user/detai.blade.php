    ...
    <div class="mb-6 bg-white p-4 rounded-md shadow">
        <form action="{{ route('detai') }}" method="GET" class="flex gap-4">
            ...
            @if(request('search_title') || request('search_year'))
                <a href="{{ route('detai') }}" class="px-6 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600">
                    Xóa bộ lọc
                </a>
            @endif
        </form>
